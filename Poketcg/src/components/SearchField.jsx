...
export default function SearchField({ tcgdex }) {
  ...
  const handleSearch = async () => {
    ...
    const results = await tcgdex.card.list(
      new Query().like('name', query).paginate(page, 5)
    );
    console.log(cards[0]);
    setCards(results);
  };
...
>   <div className="field-label" style={{ zIndex: 1 }}>Search Cards</div>

  <input
    ref={inputRef}
    className="Pass"
    placeholder="Search for cards"
    onKeyDown={(e) => e.key === "Enter" && handleSearch() && setPage(1)}
    style={{
      background: "black",
      color: "white",
      height: "20px",
  ...
  <button
    onClick={() => handleSearch() && setPage(page + 1)}
    style={{ position: "absolute", bottom: "0%", right: "10px", zIndex: 1, background: "#7F47AA", color: "white", border: "none", padding: "5px 10px", borderRadius: "4px" }}
  >next</button>
</div>
</div>
      </div>
      ...
      <div style={{ position: 'absolute', top: '25%', left: `calc(100% - ${window.innerWidth * 0.065}px)`, transform: 'translate(-50%, -50%)' }}>
        {cards.map((card, i) => (
          <CardDisplay
            key={card.id ?? i}
            imageUrl={card.image}
